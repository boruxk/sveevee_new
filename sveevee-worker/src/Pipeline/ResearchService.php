...
final class ResearchService
{
    ...
    public function __construct(
        ...
        private readonly int $businessesPerCombination = PHP_INT_MAX,
    ) {}
    ...
    public function research(string $runId, int $limit, RunReport $report): void
    {
        ...
        foreach ($this->targets as $targetIndex => $target) {
            ...
            if (! $this->targetHasCapacity($target, $categoryCounts, $neighborhoodCounts)) {
            ...
            $targetRemaining = min(
                $remaining,
                (int) ceil($remaining / $targetsLeft),
                max(1, $this->businessesPerCombination),
            );
            $targetFound = 0;
            try {
                ...
                $this->repository->markResearchTargetCompleted($target, $runId);
                $report->target($target->key(), $target->city, $target->categoryKey, $targetFound);
            }
        }
    }
