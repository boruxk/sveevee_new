...
<head>
    ...
    <title>{{ __('mail.verification.subject') }}</title>
</head>
            ...
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:600px;background:#ffffff;border:1px solid #e2e5e9;border-radius:8px;">
                <tr>
                    <td style="padding:30px;{{ $messageLocale === 'he' ? 'text-align:right;' : 'text-align:left;' }}">
                        ...
                        <h1 style="font-size:24px;line-height:1.3;margin:0 0 14px;">{{ __('mail.verification.heading') }}</h1>
                        <p style="font-size:16px;line-height:1.6;margin:0 0 24px;">{{ __('mail.verification.body') }}</p>
                        <a href="{{ $verificationUrl }}" style="display:inline-block;padding:12px 18px;border-radius:6px;background:#6d3785;color:#ffffff;text-decoration:none;font-weight:700;">{{ __('mail.verification.action') }}</a>
                        <p style="font-size:13px;line-height:1.6;color:#656b73;margin:24px 0 0;">{{ __('mail.verification.expires') }}</p>
                    </td>
                </tr>
            </table>
        </td>
