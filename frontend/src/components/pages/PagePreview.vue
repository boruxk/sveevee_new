	...
	async function copyShareLink(options = {}) {
		...
		try {
			if (navigator.clipboard?.writeText) {
				await navigator.clipboard.writeText(shareTargetUrl.value)
			} else {
				fallbackCopy(shareTargetUrl.value)
			}
				...
				>
					...
					<q-menu anchor="top right" self="bottom right" class="page-share-menu" :offset="[0, 12]">
						<div class="page-share-menu__content">
							...
							<button type="button" class="page-share-menu__button" :aria-label="t('share.copyLink')" @click="copyShareLink" v-close-popup>
